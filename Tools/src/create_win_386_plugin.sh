...
set -euo pipefail
echo "****************************************"
echo "Creating zip file for Windows 386 Plugin"
echo "****************************************"

BIN_FOLDER=${GO_SPACE}/bin
BUILD_FOLDER=${BIN_FOLDER}/windows_386_plugin
...
WINDOWS_386_ZIP=${GO_SPACE}/bin/updates/sessionmanagerplugin/`cat ${GO_SPACE}/VERSION`/session-manager-plugin-windows-386.zip
zip -j ${WINDOWS_386_ZIP} ${PACKAGE_FOLDER}/package.zip
zip -j ${WINDOWS_386_ZIP} ${PACKAGE_FOLDER}/install.bat
