...
build-darwin-arm64: checkstyle copy-src pre-build
	...
	GOOS=darwin GOARCH=arm64 $(GO_BUILD) -ldflags "-s -w -extldflags=-Wl,-z,now,-z,relro,-z,defs" -o $(GO_SPACE)/bin/darwin_arm64_plugin/session-manager-plugin -v \
		$(GO_SPACE)/src/sessionmanagerplugin-main/main.go
	GOOS=darwin GOARCH=arm64 $(GO_BUILD) -ldflags "-s -w -extldflags=-Wl,-z,now,-z,relro,-z,defs" -o $(GO_SPACE)/bin/darwin_arm64/ssmcli -v \
    		$(GO_SPACE)/src/ssmcli-main/main.go
